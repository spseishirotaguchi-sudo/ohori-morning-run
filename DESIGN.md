...
## コンセプト
朝を変えれば1日が変わる、さあ歩を進めよう。事前決済と仲間との約束で、休日の最初の一歩を支える。
...
## 確認済み条件
名称は「大濠公園朝活ラン部」。代表は田口勢士郎。
土曜または日曜、9:00〜10:00、大濠公園、1回500円、事前決済。
...
申込みは Linktree（https://linktr.ee/sp.seishiro）で受け付ける。LP上のCTAは4箇所（ヘッダー・ファーストビュー・開催情報・末尾）すべてこのURLへ外部リンクする。
開催日はLPに書かない。日程が変わるたびにLPを直す運用を避けるため、日程の掲載はLinktree側に一本化する。LPには「土曜または日曜 9:00–10:00」までしか書かない。

## 開始前の未確定項目
集合地点／走る距離／キャンセル連絡の受付方法／連絡先／決済の方法（着金の自動確認は決済事業者を通す必要がある）。
...
本文を書き換えるときは、新しい文章も <span class="ph"> で包み、文節の切れ目に <wbr> を置くこと。
包み忘れると、その部分だけ単語の途中で改行される。
